...
Для всех страниц создается микроразметка `WebPage`.


Поле **headline** используется в SEO и микроразметке:
- если отсутствует **metaDescription**, то будет взято значение **headline**;
- для всех объектов типа Страница в микроразметке `WebPage`, как поле **description**.
- для всех объектов типа Пост в микроразметке `BlogPosting`, как поле **headline**.
...
В результате обработки формы на этой странице вы получите сообщения на:

- электронную почту: `TO_EMAIL`;
- Telegram-канал: `TELEGRAM_TO`.
