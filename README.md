...
:bulb: Для полей **metaTitle** и **metaDescription** не пользуйтесь всеми заглавными буквами, т.к. они хуже читаются и занимают больше места при выводе на странице SERP.


:warning: В случае, если отсутствуют и **metaDescription** и **description**, то будет использовано соответствующее языку страницы содержимое поля **siteDescription** из файла конфигурации `[PROJECT_DIR]/config/locales.js`.
...
:bulb: Для того, чтобы в будущем было легче отслеживать переходы по внутренним ссылкам сайта в **Google Analytics**  всегда добавляйте бэк-слеш `/` в конце ссылки.
...
Если вы хотите, что бы ваш сайт был проиндексирован **Google** и другими сделайте следующее.
...
- Все страницы пагинации просматривается роботами и индексируется.
- С проблемой дублирования контента разбирается поисковик.
- `rel=next` и `rel=prev` не используются
...
# который будет используется, как шаблон страницы во время компиляции сайта
template: только латинница
...
Необязательное поле **datePublished** - дата и время публикации поста в формате ISO 8601. Посты сортируются в порядке убывания даты публикации, т.е. сначала показываются более новые.
...
# В нашем случае на Главной странице не используется, но его все равно надо указать
title: Снежный Барс
...
sections:
  ...
  - items:
    ...
    # поле text
    # Символ `|` позволяет вводить многострочные значения полей с новой строки. 
...
Список сортируется по дате публикации: поле `datePublished` из Frontmatter.
...
|   Поле       | Описание
|---           |---
| to           | путь к странице без префикса языка и без бэк-слеша в конце. Пример: `/services`
...
|   Поле       | Описание
|---           |---
| to           | путь к странице без префикса языка и без бэк-слеша в конце. Пример: `/contacts`
| title        | название ссылки
