...
- [Публикация изменений](#публикация-изменений)
  ...
  - [Обновление главного репозитория с кодом сайта](#обновление-главного-репозитория-с-кодом-сайта)
...
Сначала мы правим данные на локальном компьютере и обновляем удаленный защищенный репозиторий с данными.

Потом переходим в локальный репозиторий с кодом сайта, обновляем его из удаленного защищенного репозитория с данными и затем обновляем удаленный репозиторий с кодом.



Netlify отследит все изменения в удаленном репозитории с кодом, скомпилирует и опубликует обновленную версию сайта.

### Обновление данных

1. Отредактируйте данные сайта на локальном компьютере.
2. Запустите командную строку.
3. Перейдите в папку, где хранятся данные сайта при помощи команды `cd`.
4. `Commit` результатов редактирования данных на локальном компьютере и `push` изменения из локального репозитория в удаленный репозиторий с данными.
...
### Обновление главного репозитория с кодом сайта

1. Перейдите в папку, где хранится код сайта при помощи команды `cd`.
1. Обновите данные `submodule update` в главном репозитории с кодом на локальном компьютере из удаленного защищенного репозитория с данными, затем выполните общий `Commit`  и `push` изменений в удаленный главный репозиторий.
...
| Поле из Frontmatter | Рекомендованая длина  | Значение |Пераоисточник                                                                                                                                                                       | Комментарий                                                                |
| ----------------- | ---------| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| metaTitle         | до 50-60 символов, строка на экране будет обрезана в 600px | Главная ключевая фраза Второстепенная ключевая фраза \| Ваш бренд    | [What is a meta title tag?](https://moz.com/learn/seo/title-tag), [Create good titles and snippets in Search Results](https://support.google.com/webmasters/answer/35624?hl=en) | Если **metaTitle** отсутствует, то поле **title** будет использовано          |
| metaDescription   | 50-160 символов  | 1 параграф из 1-2 предложений. Без кавычек `"` и спецсимволоа      | [What is a meta description?](https://moz.com/learn/seo/meta-description), [How to create the right meta description](https://yoast.com/meta-descriptions/)                     | Если **metaDescription** отсутствует, то поле **description** будет использовано |
...
| OpenGraph   | og-banner-{locale}.jpg          | [PROJECT_DIR]/static/assets/images     | 1.9 x 1     | 1200 x 630 or 600 x 315 | 1 на каждый язык    | красивая картинка с наложенным лого           |
| Twitter     | twitter-banner-2x1-{locale}.jpg | [PROJECT_DIR]/static/assets/images     | 2 x 1       | 600 x 300               | 1 на каждый язык    | красивая картинка с наложенным лого           |
| Twitter     | twitter-banner-1x1-{locale}.jpg | [PROJECT_DIR]/static/assets/images     | 1 x 1       | 450 x 450               | 1 на каждый1 язык    | красивая картинка с наложенным лого           |
...
Все исходные данные сайта находятся в папке `[CONTENT_DIR]`.
...
:bulb: Для быстродействия и SEO **всегда** оптимизируйте ваши изображения в графическом редакторе до публикаци на сайте, устанвливайте их размер согласно рекомендаций этой документации и настраивайте качество.
...
Последовательность полей не сущеставенна.

Выжны отступы слева и соотношение отступов. Именно отступы задают структуру.

:bulb: Настоятельно рекомендуем ознакомится с форматом YAML до начала работы с сайтом.
...
noindex: `true` или `false` - показать или спрятать страницу для поисковых роботов
---
```

Если значение поля должно содержать двоеточие `:` или спецсимвол, то такое значение надо обрамить кавычками. Подробнее в описании формата YANL.
...
sections:
  ...
  - items:
    ...
    # title Cекции № 1 выводится с тэгом H1    
    title: НАШИ УСЛУГИ ВКЛЮЧАЮТ В СЕБЯ ВЕСЬ ЦИКЛ РАБОТ
    # поле text
    # значек `|` позволяет ввести значение поля с новой строки. Подробнее в описании формата YAML
  ...
  # title секции № 2 выводится с тэгом H2  
  ...
  # Секция № 4
  # title секции № 4 выводится с тэгом H2  
  ...
  # Секция № 5
  # title секции № 5 выводится с тэгом H2
  ...
  # Секция № 6
  # title секции № 6 выводится с тэгом H2
  - title: НАШЕ КРЕДО
    text: |
...
- выполните действия из радела **[Обновление главного репозитория с кодом сайта](#обновление-главного-репозитория-с-кодом-сайта)**.
