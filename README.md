...
1. Текстовый редактор. Рекомендуется Visual Studio Code (VSC). Ссылка для скачивания [здесь](https://code.visualstudio.com/download). Желательно дополнительно установить расширения для VSC: "YAML Language Support by Red Hat", "Markdown Preview Enhanced", "markdownlint
...
Ссылки начинающиеся с `/ru/` ведут русскоязычную версию сайта.
...
Назначение переменных в комментариях
...
#### Изображение находится рядом с самим документом в подпапке `/images`
...
#### Изображение находится в папке `/static/images` отдельно от документа
...
##### Два изображение в один ряд для десктоп и в два ряда для мобильного
...
##### Три изображение в один ряд для десктоп и в три ряда для мобильного
...
Рекомендуемый размер изображения (поле `cover`)
...
Страницы имеют индивидуальное оформление. Для решения этой задачи содержимое страницы разбивается на секции (`sections`). Каждая секция состоит из заголовка (`title`), подзаголовка (`subtitle`), текста (`text`), изображения (`image`) и массива элементов (`items`).
...
Это структурованное описание секций хранится в frontmatter в дополнение к общим полям.
...
Для сохранения ссылок на старом сайте создан постоянный редирект на эту страницу:
...
Для русскоязычных ненайденных страниц задан редирект:
...
#### Общие данные для формирования элементов сайта
...
Запрограммировано так, что первый пункт Главного меню будет **Услуги**, а второй **Типы объектов**.
