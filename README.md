...
Для указанной выше конфигурации окружения сайт будет скомпилирован для двух языков `uk` и `ru`. Будут созданы четыре страницы с адресами `/`, `/inquiry`, `/ru/`, `/ru/inquiry`. Во время генерации будут отображаться предупреждения, если есть.
