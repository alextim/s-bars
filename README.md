# s-bars.com - Снежный барс
- [s-bars.com - Снежный барс](#s-barscom---снежный-барс)
  - [Ввведение](#ввведение)
  ...
  - [Данные для формирования элементов сайта](#данные-для-формирования-элементов-сайта)
    - [Общие для всех языков](#общие-для-всех-языков)
      - [Рабочее время (openingHours)](#рабочее-время-openinghours)
...
1. Текстовый редактор. Рекомендуется Visual Studio Code (VSC). Ссылка для скачивания [здесь](https://code.visualstudio.com/download). Желательно дополнительно установить расширения для VSC: "YAML Language Support by Red Hat", "Markdown All in One", "markdownlint
...
| ---       |---                        | ---                  | ---
| 1         | Слайдер                   | ? : ?                | 1216 x ?
| 3         | Триптих                   |  1 : 1.85            | 592 x 320
| 4         | ВСЕ РАБОТЫ ОТ «А» до «Я»  | 1 : 1                | 200 x 200
| 5         | НАС РЕКОМЕНДУЮТ           | ? : ?                | 1216 x ?
...
:grey_exclamation: Для сохранения ссылок на старом сайте создан постоянный редирект на эту страницу:
...
:grey_exclamation: Для русскоязычных ненайденных страниц задан редирект:
...
| ------------ | --------------------------------------- | --------------------- | ------------------------------- |
| Контактные данные     |   .                                     | contacts.yaml         | географические координаты, ссылки на Google карты, телефоны, почта, рабочее время, диапазон цен, дата основания |

|   Поле       | Назначение                    | Описание
|---           |---                            |---
| geo          | SEO
| hasMap       | SEO
| embedMap     | страница "Контакты"
| phone        | SEO                           | список цифровых значений, разделитель элементов `,`.
| e-mail       | SEO                           | список, разделитель элементов `,`.
| openingHours | для страницы "Контакты" и SEO |
| priceRange   | SEO                           |
| foundingDate | для футера и SEO              |

:grey_exclamation: Номер телефона допускает только цифры, без плюса спереди, без пробелов, скобок и прочих символов.
...
День недели - аббревиатура из двух букв на английском языке.
...
Поле **title** - название ссылки.
Поле **to** - путь без префикса языка.
...
:grey_exclamation: Запрограммировано так, что первый пункт Главного меню будет **Услуги**, а второй **Типы объектов**.
...
:grey_exclamation: Поле **код** используется для построения SEO и для иконки ссылки на соц.сети в футере.

:bulb: Для добавления других соц.сетей нужен программист.
...
:bulb: Для автоматического обновления переводов во время `commit`-a потребуется установить **husky** и **lint-staged**.
